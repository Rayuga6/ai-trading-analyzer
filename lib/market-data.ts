...
export async function getMarketData() {
  ...
  const candles: Candle[] = await candleResponse.json();
  ...
  const resistanceLevels = recentHighs
    .filter((level) => level > currentMarketPrice)
    .sort((a, b) => a - b);
  ...
  for (
    ...
    const plusDMSlice = plusDM.slice(
      i - period + 1,
      i + 1
    );

    const minusDMSlice = minusDM.slice(
      i - period + 1,
      i + 1
    );

    const trSlice = trueRanges.slice(
      i + 1 - period,
      i + 1
    );
